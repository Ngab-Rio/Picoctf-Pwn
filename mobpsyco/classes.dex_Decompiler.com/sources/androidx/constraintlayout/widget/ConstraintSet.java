        ...
        if (data != null) {
            ...
            if ((data instanceof ConstraintLayout.LayoutParams) != 0) {
                ...
                if (orientation == 0) {
                    ...
                    params3.add((int) CONSTRAINED_WIDTH, finalConstrained);
                    return;
                }
        ...
        for (int i = 0; i < N; i++) {
            ...
            switch (overrideMapToConstant.get(attr)) {
                ...
                case 55:
                    ...
                    break;
                case WIDTH_MAX /*56*/:
                    delta.add((int) WIDTH_MAX, typedArray.getDimensionPixelSize(attr, constraint.layout.widthMax));
                    break;
                case HEIGHT_MAX /*57*/:
                    delta.add((int) HEIGHT_MAX, typedArray.getDimensionPixelSize(attr, constraint.layout.heightMax));
                    break;
                case WIDTH_MIN /*58*/:
                    delta.add((int) WIDTH_MIN, typedArray.getDimensionPixelSize(attr, constraint.layout.widthMin));
                    break;
                case HEIGHT_MIN /*59*/:
                    delta.add((int) HEIGHT_MIN, typedArray.getDimensionPixelSize(attr, constraint.layout.heightMin));
                    break;
                case 60:
                    delta.add(60, typedArray.getFloat(attr, constraint.transform.rotation));
                    break;
                case CIRCLE_RADIUS /*62*/:
                    delta.add((int) CIRCLE_RADIUS, typedArray.getDimensionPixelSize(attr, constraint.layout.circleRadius));
                    break;
                case 63:
                ...
                case 67:
                    ...
                    break;
                case PROGRESS /*68*/:
                    delta.add((int) PROGRESS, typedArray.getFloat(attr, constraint.propertySet.mProgress));
                    break;
                case WIDTH_PERCENT /*69*/:
                    delta.add((int) WIDTH_PERCENT, typedArray.getFloat(attr, 1.0f));
                    break;
                case HEIGHT_PERCENT /*70*/:
                    delta.add((int) HEIGHT_PERCENT, typedArray.getFloat(attr, 1.0f));
                    break;
                case CHAIN_USE_RTL /*71*/:
                    Log.e(TAG, "CURRENTLY UNSUPPORTED");
                    break;
                case BARRIER_DIRECTION /*72*/:
                    delta.add((int) BARRIER_DIRECTION, typedArray.getInt(attr, constraint.layout.mBarrierDirection));
                    break;
                case BARRIER_MARGIN /*73*/:
                    delta.add((int) BARRIER_MARGIN, typedArray.getDimensionPixelSize(attr, constraint.layout.mBarrierMargin));
                    break;
                case CONSTRAINT_REFERENCED_IDS /*74*/:
                    delta.add((int) CONSTRAINT_REFERENCED_IDS, typedArray.getString(attr));
                    break;
                case BARRIER_ALLOWS_GONE_WIDGETS /*75*/:
                    delta.add((int) BARRIER_ALLOWS_GONE_WIDGETS, typedArray.getBoolean(attr, constraint.layout.mBarrierAllowsGoneWidgets));
                    break;
                case PATH_MOTION_ARC /*76*/:
                    delta.add((int) PATH_MOTION_ARC, typedArray.getInt(attr, constraint.motion.mPathMotionArc));
                    break;
                case CONSTRAINT_TAG /*77*/:
                    delta.add((int) CONSTRAINT_TAG, typedArray.getString(attr));
                    break;
                case VISIBILITY_MODE /*78*/:
                    delta.add((int) VISIBILITY_MODE, typedArray.getInt(attr, constraint.propertySet.mVisibilityMode));
                    break;
                case MOTION_STAGGER /*79*/:
                    delta.add((int) MOTION_STAGGER, typedArray.getFloat(attr, constraint.motion.mMotionStagger));
                    break;
                case CONSTRAINED_WIDTH /*80*/:
                    delta.add((int) CONSTRAINED_WIDTH, typedArray.getBoolean(attr, constraint.layout.constrainedWidth));
                    break;
                case CONSTRAINED_HEIGHT /*81*/:
                    delta.add((int) CONSTRAINED_HEIGHT, typedArray.getBoolean(attr, constraint.layout.constrainedHeight));
                    break;
                case ANIMATE_CIRCLE_ANGLE_TO /*82*/:
                    delta.add((int) ANIMATE_CIRCLE_ANGLE_TO, typedArray.getInteger(attr, constraint.motion.mAnimateCircleAngleTo));
                    break;
                case TRANSFORM_PIVOT_TARGET /*83*/:
                    delta.add((int) TRANSFORM_PIVOT_TARGET, lookupID(typedArray, attr, constraint.transform.transformPivotTarget));
                    break;
                case QUANTIZE_MOTION_STEPS /*84*/:
                    delta.add((int) QUANTIZE_MOTION_STEPS, typedArray.getInteger(attr, constraint.motion.mQuantizeMotionSteps));
                    break;
                case QUANTIZE_MOTION_PHASE /*85*/:
                    delta.add((int) QUANTIZE_MOTION_PHASE, typedArray.getFloat(attr, constraint.motion.mQuantizeMotionPhase));
                    break;
                ...
                case BASELINE_MARGIN /*93*/:
                    delta.add((int) BASELINE_MARGIN, typedArray.getDimensionPixelSize(attr, constraint.layout.baselineMargin));
                    break;
                case GONE_BASELINE_MARGIN /*94*/:
                    delta.add((int) GONE_BASELINE_MARGIN, typedArray.getDimensionPixelSize(attr, constraint.layout.goneBaselineMargin));
                    break;
                ...
                case LAYOUT_WRAP_BEHAVIOR /*97*/:
                    delta.add((int) LAYOUT_WRAP_BEHAVIOR, typedArray.getInt(attr, constraint.layout.mWrapBehavior));
                    break;
                ...
                case GUIDELINE_USE_RTL /*99*/:
                    delta.add((int) GUIDELINE_USE_RTL, typedArray.getBoolean(attr, constraint.layout.guidelineUseRtl));
                    break;
                default:
